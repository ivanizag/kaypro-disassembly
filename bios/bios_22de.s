...
bios_config_6:              EQU 6 ; Modified with AENDERN8.COM
    ; 0: 7 bits mode
    ; 1: 8 bits mode
bios_config_7:              EQU 7
...
BOOT:
    ...
    DB 1Ah,"KAYPRO CP/M 2.2 {GMv2.72}\r\n",0
...
NEXT_L:
    ; Increments L, when 0, go back to 0xbd
    INC L
    LD A,L
    OR A
    RET NZ
...
OUT_EXTERNAL:
    ...
    JR Z, ADD_OFFSET
    LD HL, PRINTER_OUT_HIGH_CHAR_DE
ADD_OFFSET:
...
HAD_PENDING_ACCENT:
    ...
    LD DE, LOOKUP_ACCENT_SYMBOLS_DE
...
LOOKUP_KEY_ALT:
    ; Alt keyboard layout
    DB 0xDA,0xCE,0xD9,0xCD,0xD6,0xCC,"[]",0xde,0xdc
    ;  ü    Ü    ö    Ö    ä    Ä         ß    § 
LOOKUP_KEY2_US:
    ; US keyboard layout
    DB "@^&*()_y]}z<>/?ZY",0x19,0x1a
LOOKUP_KEY2_INTL:
    ; International keyboard layout
    DB "\"&/()=?z+*y;:-_YZ",0x1a,0x19
; Transformation of chars for printing
PRINTER_OUT_HIGH_CHAR_ALT:
    ; Translations of chars 0xc0 to 0xfd for TTY or LPT output
    DB "naeiouaeiou",0xFF,"[\\]",0xFF
    DB "Naeiou{ei|}",0xFF,"@c~",0xFF,""
PRINTER_OUT_HIGH_CHAR_DE:
    ; Translations of chars 0xc0 to 0xfd for TTY or LPT output
...
LOOKUP_ACCENT_SYMBOLS_ALT:
    ...
    DB "@@#",0xCB,"^^",0xDB,"'{}<<>>\\|"
LOOKUP_ACCENT_SYMBOLS_DE:
    ; Modifed keys on the German keyboard layout
    DB 0xFF,0xFF,"#",0xFF,"^^",0xFF,"'",0xFF,0xFF,"<<>>",0xFF,0xFF

; Transformation of chars for display accorging to the
; codepage 1011 https://en.wikipedia.org/wiki/DIN_66003
VIDEO_OUT_ESC_ORIG:
    DB "[\\]{|}@~"
VIDEO_OUT_ESC_DEST:
    DB 0xCC,0xCD,0xCE,0xD6,0xD9,0xDA,0xDC,0xDE
    ;  Ä    Ö    Ü    ä    ö    ü    §    ß
...
INPUT_BUFFER_GET_CURSOR:
    DW INPUT_BUFFER
FILLER:
    DB 0xFA,0xC0,0xFA,0x00,0x00,0x00,0xAA,0x88
    DB 0x41,0x12,0x24,0x80,0x8E,0x04,0x8E,0x04
    DB 0x02,0x04,0x88,0x9C,0xC5,0x04,0x53,0xFF
    DB 0x14,0x05,0x0f,0x04,0x28,0x08,0x62,0xDF
STACK_SAVE:
    DW 0xEB3B
FILLER2:
    DB 0x61,0x65,0x6e,0x64,0x65,0x72
    DB 0x6E,0x0D
INPUT_BUFFER:
    DB 0x48,0x21,0x20,0x08,0x22,0x21
